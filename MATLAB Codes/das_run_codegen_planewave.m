...
% GPU Coder code generation for das_beamform_planewave, targeting the
% plane wave dataset (Zenodo 7986407). Uses variable-size bounds for both
% the sample count and element count, since this dataset's array geometry
% differs from OASBUD's and the exact element count depends on which
% probe configuration was used for acquisition. Run this once real data
% is available to confirm the bounds below are generous enough.
...
fprintf('\nRunning GPU Coder on das_beamform_planewave (variable-size)...\n');
codegen('das_beamform_planewave', '-config', cfg, '-args', ...
    {arg_rf, arg_x_elements, arg_z_image, arg_x_image, arg_fs, arg_c, arg_tx_angle}, ...
